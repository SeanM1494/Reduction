...
## The Sep 8 workspace migration, and the rules it taught

The repo was restructured (outside the usual process, by Replit's Agent) into
a pnpm workspace: web frontend in `artifacts/reduction`, backend in
`artifacts/api-server`, an Expo scaffold in `artifacts/reduction-mobile`,
Drizzle schema in `lib/db`, and the recipe model in `lib/recipe-model`. The
pre-migration tree is parked in `.migration-backup/` — read-only history, not
a place to edit. Paths in this file have been updated; if a path looks wrong,
...
shipped with an auto-push line — defused Aug 3, silently re-armed by the
migration on Sep 8, at which point the next workspace merge pushed
`auth_states.redirect_uri` into the production schema with nobody running
anything. Benign that time, by luck; push can also DROP. The guard now lives
in `lib/db/drizzle.config.ts` (loopback-only unless ALLOW_REMOTE_DB_PUSH=1),
and the hook carries a comment that says why the push line must never return.
Production schema changes remain hand-run, reviewed DDL.

**Never commit `tsconfig.tsbuildinfo` (or any live state).** The migration
committed lib/db's, and the stale build-state told `tsc --build` the project
was already built — so `lib/db/dist` never emitted and the whole workspace
typecheck failed with phantom errors in files that were fine. It also
committed `.pgtest/`, the live test-database data dir, which could not even
...
- **pnpm settings live in `pnpm-workspace.yaml` and nowhere else, and
  there is deliberately NO `packageManager` field.** Newer pnpm no longer
  reads the `pnpm` field of `package.json`, and EAS Build's pnpm was the
  first to say so: it computed no `@types/pg` override, the lockfile
  recorded one, and the frozen install refused
  (`ERR_PNPM_LOCKFILE_CONFIG_MISMATCH`, Sep 18). Reconciling also showed
  the lockfile had NEVER recorded the platform exclusions the workspace
  file has carried since the migration — three environments had agreed
...
**Run `pnpm run check` and `pnpm run test:db` before every commit.** Both must
pass first. Not after, not "it typechecked earlier" — before. `test:db` is
`pnpm test` pointed at a throwaway LOCAL Postgres (`scripts/test-db.sh`: data
in `.pgtest/`, port 5433, schema re-pushed from `lib/db/src/schema/schema.ts` on every
...
`pnpm run test:db:stop` parks it, `pnpm run test:db:reset` wipes it — it is
disposable by design, so reset beats debugging it. Plain `pnpm test` still
...
`pnpm test` has exactly three legitimate outcomes, and the difference between
them is the whole point:

| result | meaning |
|---|---|
| ***n* pass, 138 skipped** | no `DATABASE_URL` at all. Fine on a machine with no Postgres. |
| ***n*+138 pass, 0 skipped** | a local database with a current schema. This is the real gate — `pnpm run test:db` produces it. |
...
edit whenever a database-backed suite is added). The 138 are fourteen suites:
...
notifications) `access.db.test.ts` (the paywall), `admin.db.test.ts` (the operator lookup), `billingApple.db.test.ts` (the App Store routes), `mobileHandoff.db.test.ts` (the mobile sign-in handoff), `account.db.test.ts` (account deletion), `photos.db.test.ts` (recipe pictures), `removed.db.test.ts` (taking a recipe out of the box) `searchLibrary.db.test.ts` (the cached half of a search and its counts) and `originals.db.test.ts` (a recipe's original wording). The first two are transactional guarantees — all-or-nothing
...
finish is proven without a device), the diagram drag's hit-testing
...
same as "safe against production". `artifacts/api-server/src/lib/testdb.ts` now refuses a
...
`pnpm run test:db` provides — nobody needs the override in normal work, and
...
`lib/db/src/schema/schema.ts` and not in the database, so all nine trial-claim tests
...
`artifacts/api-server/src/lib/testdb.ts` is now the single gate for both suites and asks the
...
`artifacts/api-server/src/lib/timerDispatch.ts` has no scheduler inside it, and that is
...
the case `lib/db/src/schema/schema.ts` cites for never looking accounts up by email.
...
match. `lib/db/src/schema/schema.ts` says plainly that nothing looks an account up by
...
On top of `pnpm run check` and `pnpm test`:
...
output.** `lib/recipe-model/src/sequence.ts` owns that, and `sequence.test.ts` asserts it
...
`lib/recipe-model/src/edits.ts` is the guard, and it works by diffing the real
...
tree to someone who asked for `?servings=8`. `artifacts/api-server/src/lib/urlKey.test.ts` has a
...
for it — see `lib/recipe-model/src/sync.ts` for the model and `sync.test.ts` for the
...
`lib/recipe-model/src/edits.ts` is the whole model: `applyEdit(recipe, op)` is pure, and one
...
identity check.** `snapQty` in `lib/recipe-model/src/amounts.ts` snaps a scaled amount to
...
- query or depend on Diagram's DOM or class names (an earlier design anchored
  ...
  to change)
- import `lib/recipe-model/src/layout.ts`
