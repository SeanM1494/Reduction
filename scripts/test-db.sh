...
# pnpm may not be on PATH for the user this runs as (a container running the
# suite as an unprivileged postgres user, say) even though node is findable.
# Resolve it next to whatever node we can see rather than failing at line 73.
if ! command -v pnpm >/dev/null 2>&1; then
  for d in "$(dirname "$(command -v node 2>/dev/null)" 2>/dev/null)" /opt/node22/bin /usr/local/bin; do
    [ -n "$d" ] && [ -x "$d/pnpm" ] && PATH="$d:$PATH" && export PATH && break
  done
fi
...
start() {
  ...
  DATABASE_URL="$URL" pnpm --filter @workspace/db run push-force >/dev/null
